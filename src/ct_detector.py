...
def load_ct_model(model_path: str = 'models/ct_model.pth'):
    model = timm.create_model('efficientnet_b0', pretrained=False, num_classes=1)
    model.load_state_dict(torch.load(model_path, map_location=DEVICE))
    model.eval()
    return model.to(DEVICE)

def predict_ct(image_input, model) -> dict:
    """
    Predict pulmonary embolism from CT scan image.
    ...
    label      = 'Clot Detected (PE)' if prob >= 0.5 else 'No Clot Detected'
